    ...
    async function handleSubmit(e){
        ...
        try{
            if(!err){
             ...
             const res= await axios.post(`${path}donor/livingdonationrequest/${user._id}`,{donationOrgans:result})
             if(res.status===200){
                navigate('/donor')
             }
            }
  ...
  return (
    ...
    <NavBar/>
        ...
        <form autoComplete='off' className='w-[23rem] pb-5 md:w-[46rem] mx-auto my-auto rounded-lg ' onSubmit={handleSubmit} style={{backdropFilter: 'blur(2px)', background: 'rgba(156, 152, 152, 0.45)'}}>
                <p className='text-[2rem] text-center font-serif cursor-default'>Living Donation </p>
